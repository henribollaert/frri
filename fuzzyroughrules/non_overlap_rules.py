...
class RuleGenerator(BaseEstimator):
    ...
    def get_rules(self, X, train_prediction):
        ...
        self.positive_region = fo.lukasiewicz_t_norm(fo.get_general_granular_approx10(init_set, self.rel_matrix_x, self.rel_matrix_y, learning_rate=.666), .5)
        ...
        for i in range(self.n_samples):
            ...
            covering.append(1*(current_covering > 1e-6))
        self.selected_indexes = self.__optimisation_procedure(np.array(covering).T)
